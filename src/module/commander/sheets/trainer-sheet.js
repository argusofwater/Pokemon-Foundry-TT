...
export class CommanderTrainerSheet extends CommanderActorSheetBase {
  ...
  async _prepareContext(options) {
    ...
    for (const uuid of this.actor.system.team?.pokemonUuids ?? []) {
      ...
      team.push({
        ...
        friendship,
        hpPercent: pokemon.system.health?.hp?.max
          ? Math.round((Number(pokemon.system.health.hp.value ?? 0) / Number(pokemon.system.health.hp.max)) * 100)
          : 0
      });
    }
  ...
  async _onDropActor(event, data) {
    ...
    await dropped.update({
      "system.identity.trainerUuid": this.actor.uuid,
      "system.identity.lifecycle": current.length ? "party" : "active"
    });

    if (!this.actor.system.team?.activePokemonUuid) {
      await this.actor.update({ "system.team.activePokemonUuid": dropped.uuid });
    }
  ...
  static async setActivePokemon(event, target) {
    ...
    const updates = [app.actor.update({ "system.team.activePokemonUuid": uuid })];
    updates.push(next.update({
      "system.identity.trainerUuid": app.actor.uuid,
      "system.identity.lifecycle": "active"
    }));
  ...
  static async removePokemon(event, target) {
    ...
    await app.actor.update({
      "system.team.pokemonUuids": remaining,
      "system.team.activePokemonUuid": replacementUuid
    });

    if (pokemon?.type === "pokemon" && pokemon.system.identity?.trainerUuid === app.actor.uuid) {
      await pokemon.update({
        "system.identity.trainerUuid": "",
        "system.identity.lifecycle": "reserve"
      });
    }
